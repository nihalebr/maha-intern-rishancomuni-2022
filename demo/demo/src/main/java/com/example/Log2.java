...
public class Log2 {
	  ...
	  public static void DateExtraction(String regex) throws SQLException,ClassNotFoundException {
        ...
        String sql = "CREATE TABLE IF NOT EXISTS LOG2("
        + "ClientIp TEXT,"
        + "URL TEXT,"
        + "SessionId TEXT);";
        stmt.execute(sql);
            ...
            //  sorting load time in descending order
             for(int i = 0;i <data.toArray().length;i++) {
               for(int j = i;j < data.toArray().length;j++){
                    if(Integer.parseInt(data.get(i).get(12).toString().substring(1, data.get(i).get(12).toString().length()-1)) < Integer.parseInt(data.get(j).get(12).toString().substring(1, data.get(j).get(12).toString().length()-1))) {
             ...
             for(int i = 0;i < data.toArray().length;i++){
                ...
                sql = "INSERT INTO LOG2 VALUES(";
                  for(int j =0;j<temp.size();j++) {
                    if(j == 2 || j == 6 || j == 14)
                      sql = sql + "'"+  temp.get(j).substring(1, temp.get(j).length()-1) + "',";
                  }
                    sql = sql.substring(0, sql.length()-1) + ");";
                    System.out.println(sql);
                    stmt.execute(sql);
