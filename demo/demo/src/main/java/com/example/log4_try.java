...
public class log4_try {
    public static void MaxTime() throws SQLException,ClassNotFoundException {
        ...
        String qryHighTimeUrl = "select clientip,sessionid,url,timetaken_in_ms from log where clientip='"+rs.getString(1)+"' and sessionid='"+rs.getString(2)+"' order by timetaken_in_ms desc;";
        ...
        while (rs1.next()) {
            ...
            for (int i = 0; i < ip.length; i++) {
                String qryHighTimeString = "select clientip,sessionid,url,timetaken_in_ms from log where clientip='"+ip[i]+"' and sessionid='"+ id[i] +"' order by timetaken_in_ms desc;";
